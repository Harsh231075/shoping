    ...
    <>
      ...
      <Carousel responsive={responsive}>
        {Electronics.slice(0, 5).map((items) => {
          return (
            <div className="border p-2 sm:p-3 md:mx-2 mt-5">
              <React.Fragment key={items._id}>
                <Link to={`/details/${items._id}`}>
                  <img
                    className="rounded-t-lg w-full h-40 sm:h-48 md:h-60"
                    src={items.images[0]}
                    alt="product image"
                  />
                </Link>
                <div className="px-3 sm:px-5 pb-3 sm:pb-5">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button className="w-16 sm:w-20 md:w-40 py-2 text-xs sm:text-sm md:text-base text-white rounded-lg bg-blue-500 focus:outline-none">
                      Buy
                    </button>
                  </div>
                </div>
